...
const items = [
  ...
  { href: "/movies", label: "Movies", center: true, svg: (
    <svg width="26" height="26" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5.14v14l11-7-11-7z"/></svg>
  )},
        ...
        {items.map(i=>{
          ...
          return (
            <Link key={i.href} href={i.href} className={`bottom-item ${active?"active":""} ${i.center?"center":""}`}>
              <span className={`bottom-icon ${i.center?"center-icon":""}`}>{i.svg}</span>
